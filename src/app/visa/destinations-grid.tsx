  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-12">
      {destinations.map((destination) => {
        const targetUrl = destination.hasActiveVisas 
          ? (from ? `/visa/${from.toUpperCase()}-to-${destination.code}` : `/visa-search?to=${destination.code}`)
          : '#';
          ...
          <Link
            ...
            className={`group bg-white rounded-2xl p-5 shadow-md hover:shadow-xl transition-all duration-300 border ${
              destination.hasActiveVisas 
                ? 'border-slate-200 hover:border-violet-300 hover:-translate-y-1 cursor-pointer' 
                : 'border-slate-100 opacity-75 cursor-not-allowed pointer-events-none'
            }`}
          >
