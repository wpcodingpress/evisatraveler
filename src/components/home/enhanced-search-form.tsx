...
export function EnhancedSearchForm({ countries }: SearchFormProps) {
  ...
  // Load destinations when from country changes
  useEffect(() => {
    if (fromCountry) {
      setLoadingDestinations(true);
      fetch(`/api/destinations?from=${fromCountry}`)
        .then(res => res.json())
        .then(data => {
  ...
  const validateForm = () => {
    ...
    if (!fromCountry) newErrors.from = 'Select your nationality';
  ...
  const selectedTo = destinations.find(d => d.code === toCountry);

  const DropdownModal = ({ type }: { type: 'from' | 'to' }) => {
    const isFrom = type === 'from';
    const selected = isFrom ? fromCountry : toCountry;
    const currentList = isFrom ? filteredCountries : filteredDestinations;
    const isLoading = isFrom ? false : loadingDestinations;
        ...
        >
          <div className={`p-4 border-b border-slate-100 ${isFrom ? 'bg-violet-50' : 'bg-emerald-50'}`}>
            <div className="flex items-center justify-between mb-3">
              ...
              {!isFrom && !fromCountry && (
                <span className="text-sm text-slate-500">Select nationality first</span>
              )}
              <button
                onClick={() => { setActiveDropdown(null); setSearchQuery(''); }}
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-slate-500">Select your nationality first</p>
                <p className="text-slate-400 text-sm">Available destinations will appear here</p>
              </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!isFrom && 'minPrice' in item && (
                      <span className="text-sm text-slate-500">From ${(item as any).minPrice}</span>
                    )}
                  </div>
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-slate-500">No {isFrom ? 'countries' : 'destinations'} found</p>
        ...
        <div className="p-4 sm:p-6">
            ...
            <div>
              ...
              <div className="relative">
                <button
                  ...
                  className={`w-full h-14 flex items-center gap-3 bg-white border-2 ${errors.to ? 'border-red-400 bg-red-50' : !fromCountry ? 'border-slate-100 bg-slate-50 cursor-not-allowed' : 'border-slate-200 hover:border-emerald-300 hover:bg-emerald-50'} rounded-xl px-4 text-left transition-all duration-200`}
                >
                  {selectedTo ? (
                    <>
                    ...
                    <span className={`text-sm flex-1 ${!fromCountry ? 'text-slate-300' : 'text-slate-400'}`}>
                      {!fromCountry ? 'Select nationality first' : 'Select destination...'}
                    </span>
                  )}
                  <svg className={`w-5 h-5 ${!fromCountry ? 'text-slate-300' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
              </div>
          ...
          <div className="mt-6">
            <button
              ...
              className={`w-full h-14 rounded-xl font-bold text-white ${!fromCountry || !toCountry ? 'bg-slate-300 cursor-not-allowed' : 'bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500'} transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2`}
            >
